...
项目监控与风险联动编排 Skill 将项目执行数据采集、偏差分析、预警评估、风险应对串联为闭环，确保项目执行全程可视化、风险可知可控。
## When to use
...
## 信息采集（Step 0 — 项目监控编排前的上下文检查）

> ⚠️ **执行前必读**：本编排Skill需要明确的监控周期和项目基线。信息缺失时应主动采集。
...
| 项目基线（范围/进度/成本基线） | 请告知 | 先调用其他Skill建立基线 |
| 监控周期 | 请告知 | 默认每周五执行 |
| 健康度阈值 | 请告知 | 使用默认值（SPI/CPI 🟢≥0.95 🟡0.90-0.94 🔴<0.90） |
