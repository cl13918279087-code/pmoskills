...
## 信息采集（Step 0 — 项目关闭前的条件检查）

> ⚠️ **执行前必读**：项目关闭有严格的入口条件，不满足条件而提前执行会导致审计追踪不完整。

### Step 0.1 — 关闭条件检查

| 数据项 | 是否满足 | 说明 |
|--------|---------|------|
| 所有里程碑已签收 | 请告知 | 标注 `[假设值-请核对]` |
| 缺陷已全部关闭 | 请告知 | 标注 `[假设值-请核对]` |
| 文档已归档 | 请告知 | 标注 `[假设值-请核对]` |
| 项目验收已批准 | 请告知 | 标注 `[假设值-请核对]` |
| 风险/问题已关闭 | 请告知 | 标注 `[假设值-请核对]` |
| 最终EVM数据已采集 | 请告知 | 标注 `[假设值-请核对]` |
